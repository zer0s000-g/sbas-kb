...
The current source-hardening work has produced five high-value outcomes:
...
5. **NEW — 2026-05-03**: [[Source - ICAO APAC GBAS-SBAS Implementation Forums]] is elevated from starter scaffold to partially-verified regional coordination anchor, with ITF/7 (May 2025, A3-WP05 rev3) as the first confirmed public ICAO APAC SBAS working paper. Confirmed signals: LP/LPV/LNAV/VNAV/LNAV operational types, Class 3/4 SBAS avionics, six-year advance-notice requirement, SBAS architecture components.
...
| [[Source - SBAS Service Providers]]                    | SBAS service-provider source family; WAAS (FAA), EGNOS (EUSPA/ESA), MSAS (JCAB), GAGAN (AAI/ISRO), BDSBAS (China), KASS (KARI), SouthPAN (GA/LINZ)     | Service-provider routing anchor for system identity, coverage, availability, and service commitments                                               | Comparative performance tables without verified service-provider data; interoperability claims without bilateral agreements                                |
...
| System comparisons                 | Service definitions and performance reports from each system; [[Source - SBAS Service Providers]] | Draft/synthesis in many pages; service-provider source note now exists                  | Compare source posture first; avoid false precision; link to service-provider source note for system identity                                     |
...
5. ~~Create service-provider source notes for WAAS, EGNOS, MSAS, GAGAN, BDSBAS, KASS, SouthPAN, and other systems before publishing comparative performance tables.~~ **Partially completed 2026-05-03**: service-provider routing anchors exist for all major systems; deep-dive extraction from service-provider documentation still required.
6. **NEW — Priority in next cycle**: Directly extract [[Source - ICAO APAC GBAS-SBAS Implementation Forums]] for ITF/7 full meeting page and remaining ITF/6 through ITF/1 working papers; this is the highest-value regional governance signal for ASEAN SBAS planning.
7. Only after the above, reintroduce numerical thresholds or performance values into concept and aviation pages.
